...
fig = figure('Visible', 'off', 'Position', [100 100 620 900]);
...
for m = 1:numMeses
    if pMMD(m) < 0.05
        text(m, mmdVals(m) + 0.05*max(mmdVals), '*', 'HorizontalAlignment', 'center', 'FontSize', 16, 'FontWeight', 'bold');
    end
end
hold off;
...
for m = 1:numMeses
    if pDomain(m) < 0.05
        text(m, aDistVals(m) + 0.05*max(aDistVals), '*', 'HorizontalAlignment', 'center', 'FontSize', 16, 'FontWeight', 'bold');
    end
end
hold off;
xticks(1:numMeses); xticklabels(monthLabels);
...
sgtitle('Covariate shift in the representation space (128-d)', 'FontSize', 11);
