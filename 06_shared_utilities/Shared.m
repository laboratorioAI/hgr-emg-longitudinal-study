...
classdef Shared
    ...
    properties (Constant)
        ...
        WINDOW_STEP_TRF = 45; % 15 estaba 30
        % #################################################################
        % #################################################################
        ...
        % Recognition
        FRAME_CLASS_THRESHOLD = 0.25; % 0.75 0.25;
        ...
        % User distribution
        includeTesting = true; %false true
        numTestUsers = 16;

        % ####################### Drift temporal (Mes1..Mes6) #############
        % Cuantos meses de "test" (Mes1..Mes6) tienen anotacion consistente
        % con Mes0 (groundTruthIndex presente) y por lo tanto son validos
        % para la comparacion de drift/paradigmas. Mes1, Mes5 y Mes6 fueron
        % completados via segmentacion manual de estudiantes (ver
        % addGroundTruthFromStudents.m y segmentacion-metodos-numericos\) y
        % validados al 100% (57/57, 36/36, 19/19 usuarios respectivamente) el
        % 2026-07-14 -- ya tienen groundTruthIndex igual que Mes0-Mes4. Este
        % es el UNICO lugar que hay que tocar para incluirlos en
        % trainSoftmaxHead.m, context_bandit.m, context_bandit_adaptive.m y
        % compareParadigms.m. Unico hueco pendiente: 7 usuarios de Mes1
        % (36,48,50,52,54,60,64) sin archivos crudos todavia.
        NUM_VALID_TEST_MONTHS = 6;
    end
