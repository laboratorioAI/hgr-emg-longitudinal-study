...
Trains the CNN-Transformer-encoder backbone (the paper's only architecture) at three capacities, then
freezes each and caches its representation across all seven monthly sessions.
...
2. **`trainBackboneTCN.m`** -- trains an alternative temporal-convolutional-network backbone, used as an
   architecturally-distinct sanity check in earlier, exploratory stages of this project. It is not part
   of the paper's reported comparison (which uses only the Transformer-encoder backbone, at three
   capacities); kept here for reference in case you want to extend the comparison yourself.
...
3. **`buildFeatureCache.m`** -- takes a backbone trained by step 1 (auto-detects the most recent
   ...
   representation. This cache is what every decision head in `03_decision_heads/` consumes; the backbone
   itself is never retrained again after this step.
